...
Ranks all active incidents by a composite priority score that factors in:
  - Severity (critical=40, high=30, moderate=20, low=10)
  - Spread risk (extreme=30, high=20, moderate=10, low=5)
  - Structures threatened (up to 20 pts)
  - Containment (inverse — higher containment = lower priority)
  - Available resources on scene (under-resourced incidents rank higher)
  - Rate of growth signal (acres_burned vs time)

Also recommends which available units to move to which incident, so the
IC can make cross-incident resource allocation decisions quickly.
"""
...
def _priority_score(incident: Incident, db: Session) -> dict:
    """Compute a 0-100 priority score for a single incident."""
    score = 0.0
    factors: list[str] = []
    ...
    # Fire type bonus (0–10)
    score += FIRE_TYPE_BONUS.get((incident.fire_type or "wildland").lower(), 0)

    # Structures threatened (0–20, log-scaled to avoid runaway)
    ...
    # Containment penalty — well-contained incidents need fewer resources (up to -15)
    containment = incident.containment_percent or 0
    contain_penalty = -(containment / 100.0) * 15
    score += contain_penalty

    # Under-resourcing bonus (0–10)
    ...
    # Recent hours age of incident — older incidents have lower urgency (up to -10)
    age_hrs = 0.0
    ...
    # High wind + low humidity = rapidly evolving (0–10 bonus)
...
def _recommend_allocation(
    ...
    """
    Simple greedy allocation: assign the closest available unit of each
    needed type to the highest-priority under-resourced incident.
    """
    allocations: list[dict] = []
    pool = list(available_units)

    for inc in ranked:
        # Only fill incidents with < 3 units up to top-3 priority incidents
        ...
        for unit in pool[:]:
            ...
            if dist_km > 150:  # don't recommend moves > 150 km
                continue
            allocations.append({
...
def get_multi_incident_priority(
    ...
    """
    Returns incidents ranked by composite priority score (severity, spread,
    structures, containment, resources, weather) with cross-incident
    ...
    ranked = [_priority_score(inc, db) for inc in incidents]
